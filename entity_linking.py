...
from REL.ner import NERBase, Span, Cmns
...
class NERSpacy(NERBase):
    ...
    def predict(self, snt, processed_sentences):
        mentions = []
        for ent in snt:
...
def rel_entity_linking(spacy_docs, rel_base_url, rel_wiki_version, rel_ed_model_path):
    ...
    tagger_ngram = Cmns(rel_base_url, rel_wiki_version, n=5)
    temp_spacy_docs = dict()
    for key in spacy_docs:
        temp_spacy_docs[key] = [spacy_docs[key].text, []]
    mentions_dataset, n_mentions = mention_detection.find_mentions(temp_spacy_docs, tagger_ngram)
    # mentions_dataset, _ = mention_detection.format_spans(temp_spacy_docs)
    # mentions_dataset, _ = mention_detection.find_mentions(temp_spacy_docs, tagger_spacy)
    config = {
        'mode': 'eval',
    ...
    linked_entities = process_results(mentions_dataset, predictions, temp_spacy_docs)
